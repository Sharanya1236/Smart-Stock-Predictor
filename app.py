...
with col2:
    st.markdown("""
        ...
        <p style='font-size: 18px; color: grey; margin-top: 2px;'>Your AI-powered stock forecasting companion</p>
...
if st.button("🔮 Predict Next Days"):
    if ticker.strip() == "":
        ...
        with st.spinner("Fetching data and generating predictions..."):
            ...
            if data.empty:
                st.error("No data found for the given ticker. Please check the symbol and market.")
            else:
                # Historical chart
                ...
                # Combined matplotlib chart
                st.subheader("📊 Combined View")
                plt.figure(figsize=(12, 6))
                plt.plot(data.index, data['Close'], label='Historical', linewidth=2)
                plt.plot(pred_df.index, pred_df['Predicted Close'], label='Predicted', marker='o', linestyle='--', color='orange')
                plt.title(f"{ticker} Closing Price Forecast", fontsize=16)
                plt.xlabel("Date")
                plt.ylabel("Price")
                plt.grid(True)
                plt.legend()
